...
| Chat route body   | **Outer shape**: thin Zod guard (`z.object({ messages: z.array(z.unknown()) })`). **`UIMessage[]` content**: use `validateUIMessages({ messages, tools })` from the AI SDK — Zod alone cannot faithfully reproduce the `UIMessage` shape across SDK versions. Return 400 on failure. ([web/ai-streaming.md](../web/ai-streaming.md)) |
| AI SDK tool args  | Zod-parse inside `execute` at the tool boundary — LLM-emitted args are untrusted. Also validate again in the Elysia route.                                                                                                                                                                                                           |
