...
- Wrap throwing third-party calls (Web MCP tool `execute`, persistence, an optional Elysia/Eden backend) at the boundary with `Result.try` or `Result.tryPromise`.
- Keep truly exceptional programmer defects as throws or framework control flow. Do not convert every throw blindly.
- The Vercel AI SDK manages its own streaming errors — surface them via `useChat`'s `error`; do not wrap the stream itself (see [../web/ai-streaming.md](../web/ai-streaming.md)).

```typescript
const result = await Result.tryPromise({
  try: () => addTask(input.title), // e.g. inside a Web MCP tool's execute
  catch: (cause) => new ToolError({ cause, message: "Tool execution failed" }),
});
```
